export const QUIZ_QUESTIONS = [
  ...
  },
  {
    ...
    question: "What's your biggest frustration with healthy snack bars?",
    options: [
      { label: 'Midday energy crashes', val: 'Midday energy crashes' },
      { label: 'Hidden sugar traps', val: 'Hidden sugar traps' },
      { label: 'Chalky, artificial taste', val: 'Chalky, artificial taste' },
      { label: 'Bloating and indigestion', val: 'Bloating and indigestion' }
    ]
  },
  {
    id: 'dream_flavor',
  ...
  },
  {
    ...
    question: 'What do you look for in a snack bar:',
    options: [
      { label: 'Zero Added Sugar', val: 'Zero Added Sugar' },
      { label: '100% Plant Based', val: '100% Plant Based' },
      { label: '100% Whey based', val: '100% Whey based' },
      { label: 'Strictly Gluten Free', val: 'Strictly Gluten Free' },
      { label: 'High Protein (15g+)', val: 'High Protein (15g+)' }
    ]
  },
  {
    id: 'hunger_monster_time',
    question: 'When does your hunger monster wake up?',
    options: [
      { label: 'The 11 AM Slump', val: 'The 11 AM Slump' },
      { label: 'The 4 PM Wall', val: 'The 4 PM Wall' },
      { label: 'Late Night Cravings', val: 'Late Night Cravings' },
...
export function calculateArchetype(answers) {
  ...
  if (hustle.includes('Fitness') || preference.includes('High Protein') || hungerTime.includes('Workout')) {
    return ARCHETYPES[2];
  }
  if (preference.includes('Zero Added Sugar') || preference.includes('Plant Based') || preference.includes('Gluten Free')) {
    return ARCHETYPES[1];
  }
  if (hungerTime.includes('4 PM') || hustle.includes('Office') || hustle.includes('Work From Home')) {
    return ARCHETYPES[0];
  }
  return ARCHETYPES[3];
}
