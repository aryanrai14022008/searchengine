...
export async function deleteSubmissionById(id) {
  ...
  if (db) {
    try {
      ...
      const deleteQuery = isObjId 
        ? { $or: [{ _id: objId }, { _id: id }] } 
        : { _id: id };

      await db.collection(RESPONSES_COLLECTION).deleteMany(deleteQuery);
      await db.collection(CONTACT_COLLECTION).deleteMany(deleteQuery);
    } catch (e) {
  ...
  try {
    const localItems = getLocalData().filter(item => String(item._id) !== String(id));
    saveLocalData(localItems);
  } catch (err) {}
